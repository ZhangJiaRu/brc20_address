...
# 辅助函数：获取持有者列表
def get_holders(ticker, total_supply):
    url = f"https://open-api.unisat.io/v1/indexer/brc20/{ticker}/holders"
    headers = {
        "Authorization": f"Bearer {UNISAT_API_KEY}"
    }
    try:
        response = requests.get(url, headers=headers)
        if response.status_code == 200:
            holders = response.json()['data']['detail']
            # 批量获取地址
...
def token_detail(ticker):
    ...
    # print(f"获取铭文详情: {token_info}")
    holders = get_holders(ticker, int(token_info['max']))
    return render_template('token_detail.html', token=token_info, holders=holders, ticker=ticker)

# 路由：地址详情
@app.route('/address/<address>')
def address_detail(address):
    address_info = get_address_info(address)
    # print(f"获取地址详情: {address_info}")
    if not address_info:
        ...
        if 'bulk_import' in request.form:
            ...
            if address:
                ...
                if existing:
                    ...
                    db.session.add(new_remark)
                db.session.commit()
        return redirect(url_for('address_detail', address=address))  # 提交后返回地址详情页
    all_remarks = AddressRemark.query.all()
    prefill_address = request.args.get('address', '')  # 支持跳转式预填
    return render_template('remarks.html', remarks=all_remarks, prefill_address=prefill_address)
